...
int main() {
  ...
  for (int episode{}; episode < LEARNING_EPISODES; ++episode) {
    ...
    while (state != NUM_STATES - 1) {
      ...
      double max_next_q =
          std::max(q_table(next_state, 0), q_table(next_state, 1));
      q_table(state, action) +=
          LR * (reward + DISCOUNT_FACTOR * max_next_q - q_table(state, action));
      state = next_state;
    }
  }
  ...
  if (rc) {
    std::cerr << "Unable to open the database, sqlite error: "
              << sqlite3_errmsg(db) << std::endl;
  }
  const char* sql_create =
      "CREATE TABLE IF NOT EXISTS q_table (state INT, action INT, q_value "
      "REAL);";
  rc = sqlite3_exec(db, sql_create, 0, 0, &errMsg);
  if (rc != SQLITE_OK) {
    std::cerr << "SQL error while trying to create table, sqlite error: "
              << errMsg << std::endl;
  } else {
    ...
    for (int s{}; s < NUM_STATES; ++s) {
      for (int a{}; a < NUM_ACTIONS; ++a) {
        std::string sql_insert =
            ...
            std::to_string(s) + ", " + std::to_string(a) + ", " +
            std::to_string(q_table(s, a)) + ");";
